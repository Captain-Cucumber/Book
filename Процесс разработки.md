...
*Изменив данную строчку и скомпилировав демон монеты заново вы уже не сможете участвовать в действующей сети. С этого момента начинается создание собственной сети электронной валюты данного вида.
Используя в качестве параметра новость при создании Genesis block’а вы (в том числе) получаете неопровержимое доказательство о дате начала работы вашей сети.*
...
**редактирование производилось в Ubuntu.** Чтобы сгенерировать это число в терминале пишем:
...
В терминале переходим в директорию монеты `yourcoin/src/`
...
Открываем проводник, переходим в домашний каталог, используем комбинацию клавишь Ctrl+H, находим каталог `.yourcoin`, файл `debug.log` (home/.yourcoin/debug.log)

Копируем последнюю строку, возвращаемся в файл `main.cpp` и изменяем строку:

	assert(block.hashMerkleRoot == uint256("<>")ur

Копируем следующий блок кода: *он отвечает за генерацию генезис блока*
...
В терминале выполняем компиляцию и запускаем демона для testnet-сети.
...
Возвращаемся в `debug.log` testnet-сети (home/.yourcoin/testnet3/debug.log).
Находим строку
...
Возвращаемся в терминал выполняем компиляцию и запуск демона уже для mainnet-сети.
...
меняем на
	...
	uint256 hashGenesisBlock("0x<вставляем скопированное значение>");

Сохраняем *и вообще повнимательнее с сохранениями*

### Удаление чекпоинтов

В проводнике переходим в `yourcoin/src/` и открываем файл `checkpoints.cpp`
...
в 3 строке блока, 1500 меняем на 0, вместо буквенноцифрового значения вставляем скопированное ранее
приводим блок кода, приведенный выше, к следующему виду

	static MapCheckpoints mapCheckpoints =
        boost::assign::map_list_of
        (  0, uint256("0x<вставляем скопированное значение>"))
		; 
    static const CCheckpointData data = {

Возвращаемся в `main.cpp` строка `2794`

	block.nTime    = 1516978950; // копируем значение после "="

Возвращаемся в файл `checkpoints.cpp` и переходим к следующему блоку кода:
...
Смотрим что появилось в текущей директории:
	...
	ls //отобразится содержимое директории в которой вы находитесь, ищем <yourcoin>-qt !!!!

Делаем пробный запуск кошелька *для testnet-сети с флагом `-testnet`, для mainnet-сети флаг не нужен*
...
Далее в основной машине переходим в `.yourcoin`, которая также находится в домашней директории. И также создаем файл `yourcoin.conf` и вставляем туда:

	addnode=<ip склонированной машины>

*Для того чтобы узнать <ip склонированной машины>*

Теперь на обоих машинах запускаем кошельки, для этого переходим в папку с нашей монетой и находим файл `yourcoin-qt`
